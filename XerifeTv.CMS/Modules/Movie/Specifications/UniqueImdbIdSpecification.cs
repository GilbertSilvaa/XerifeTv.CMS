...
public class UniqueImdbIdSpecification(IMovieRepository repository) : ISpecification<MovieEntity>
{
    public async Task<bool> IsSatisfiedByAsync(MovieEntity movie)
    {
        try
        {
            var movieByImdb = await repository.GetByImdbIdAsync(movie.ImdbId);
